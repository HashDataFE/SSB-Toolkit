...
# constants
COPY_IMPORT="copy"
GPFDIST_IMPORT="gpfdist"
ssb_database_name=""

# default settings
import_type=$COPY_IMPORT # must be one of [copy, gpfdist]
scale="1" # scale of dataset, and located under generated/scale-${scale}-data/

function show_help()
{
    cat << EOF
Import generated dataset import database via COPY, gpfdist or external tables.
...
Args:
   ...
   -t [import_type]
    Optional, import type, must be one of copy|gpfdist, default is gpfdist

Usage:
    Import 1G dataset into target Cloudberry:
        ./import_data.sh -s 1

    Import 1G dataset into target Cloudberry via copy:
        ./import_data.sh -s 1 -t copy

    Import 1G dataset into a custom database:
        ./import_data.sh -s 1 -D mydatabase_1g
    
    Below environment variables are also supported to allow you to connect to a custom Cloudberry cluster:
...
if [ "$import_type" = "${gpfdist_IMPORT}" ]; then
    do_gpfdist_import "${ssb_database_name}" "${datadir}" "${import_data_dir}"
fi
