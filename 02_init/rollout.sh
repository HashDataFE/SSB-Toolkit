...
function copy_config() {
  ...
  if [ -n "${MASTER_DATA_DIRECTORY}" ]; then
    cp "${MASTER_DATA_DIRECTORY}/pg_hba.conf" "${SSB_DIR}/log/pg_hba_${DB_VERSION}.conf"
    cp "${MASTER_DATA_DIRECTORY}/postgresql.conf" "${SSB_DIR}/log/postgresql.conf_${DB_VERSION}.conf"
  elif [ -n "${COORDINATOR_DATA_DIRECTORY}" ]; then
    cp "${COORDINATOR_DATA_DIRECTORY}/pg_hba.conf" "${SSB_DIR}/log/pg_hba_${DB_VERSION}.conf"
    cp "${COORDINATOR_DATA_DIRECTORY}/postgresql.conf" "${SSB_DIR}/log/postgresql.conf_${DB_VERSION}.conf"
  else
  ...
  psql ${PSQL_OPTIONS} -v ON_ERROR_STOP=1 -q -A -t -c "SELECT * FROM gp_segment_configuration" -o "${SSB_DIR}/log/gp_segment_configuration_${DB_VERSION}.txt"
}
