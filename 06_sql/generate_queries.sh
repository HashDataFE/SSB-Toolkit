...
rm -f ${SSB_DIR}/06_sql/*.${BENCH_ROLE}.*.sql*
...
for p in $(seq 1 99); do
  ...
  log_time "Creating: ${SSB_DIR}/06_sql/${filename}"
  printf "set role ${BENCH_ROLE};\nset search_path=${DB_SCHEMA_NAME},public;\n" > ${SSB_DIR}/06_sql/${filename}

  # Set optimizer settings
  for o in $(cat ${SSB_DIR}/01_gen_data/optimizer.txt); do
  ...
  sed -n ${start_position},${end_position}p ${PWD}/query_0.sql >> ${SSB_DIR}/06_sql/${filename}

  # Check database if postgresql then comment out optimizer settings
  if [ "${DB_VERSION}" == "postgresql" ]; then
    sed -i 's/^set optimizer=.*/-- &/' "${SSB_DIR}/06_sql/${filename}"
    sed -i 's/^set statement_mem=.*/-- &/' "${SSB_DIR}/06_sql/${filename}"
  fi
  
  query_id=$((query_id + 1))
  file_id=$((file_id + 1))
  log_time "Completed: ${SSB_DIR}/06_sql/${filename}"
done
