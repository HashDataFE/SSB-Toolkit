...
function get_count_generate_data() {
  ...
  done < ${SSB_DIR}/segment_hosts.txt
}
...
function kill_orphaned_data_gen() {
  ...
  for i in $(cat ${SSB_DIR}/segment_hosts.txt); do
...
function gen_data() {
  ...
  for i in $(psql ${PSQL_OPTIONS} -v ON_ERROR_STOP=1 -q -A -t -c "${SQL_QUERY}"); do
    ...
    GEN_DATA_PATH="${GEN_DATA_PATH}/ssbenchmark"
    echo "ssh -n ${EXT_HOST} \"bash -c 'cd ~/; ./generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_PATH} > /tmp/ssb.generate_data.${CHILD}.log 2>&1 &'\""

    ssh -n ${EXT_HOST} "bash -c 'cd ~/; ./generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_PATH} > /tmp/ssb.generate_data.${CHILD}.log 2>&1 &'" &
  done
  wait
}
...
if [ "${GEN_NEW_DATA}" == "true" ]; then
  if [ "${RUN_MODEL}" != "local" ]; then
    ...
    while [ ${CHILD} -le ${PARALLEL} ]; do
      ...
      log_time "${GEN_DATA_PATH}/${CHILD}/dbgen -f -s ${GEN_DATA_SCALE} -C ${PARALLEL} -S ${CHILD} -T c > ${GEN_DATA_PATH}/logs/ssb.generate_data.${CHILD}.log 2>&1 &"
      ${GEN_DATA_PATH}/${CHILD}/dbgen -f -s ${GEN_DATA_SCALE} -C ${PARALLEL} -S ${CHILD} -T c > ${GEN_DATA_PATH}/logs/ssb.generate_data.${CHILD}.log 2>&1 &
      log_time "${GEN_DATA_PATH}/${CHILD}/dbgen -f -s ${GEN_DATA_SCALE} -C ${PARALLEL} -S ${CHILD} -T s >> ${GEN_DATA_PATH}/logs/ssb.generate_data.${CHILD}.log 2>&1 &"
      ${GEN_DATA_PATH}/${CHILD}/dbgen -f -s ${GEN_DATA_SCALE} -C ${PARALLEL} -S ${CHILD} -T s >> ${GEN_DATA_PATH}/logs/ssb.generate_data.${CHILD}.log 2>&1 &
      log_time "${GEN_DATA_PATH}/${CHILD}/dbgen -f -s ${GEN_DATA_SCALE} -C ${PARALLEL} -S ${CHILD} -T l >> ${GEN_DATA_PATH}/logs/ssb.generate_data.${CHILD}.log 2>&1 &"
      ${GEN_DATA_PATH}/${CHILD}/dbgen -f -s ${GEN_DATA_SCALE} -C ${PARALLEL} -S ${CHILD} -T l >> ${GEN_DATA_PATH}/logs/ssb.generate_data.${CHILD}.log 2>&1 &   
      CHILD=$((CHILD + 1))
    done
    wait
    CHILD=1
    while [ ${CHILD} -le ${PARALLEL} ]; do  
      if [ "$CHILD" -eq "1" ]; then
        ...
        log_time "${GEN_DATA_PATH}/${CHILD}/dbgen -f -s ${GEN_DATA_SCALE} -T p >> ${GEN_DATA_PATH}/logs/ssb.generate_data.${CHILD}.log 2>&1 &"
        ${GEN_DATA_PATH}/${CHILD}/dbgen -f -s ${GEN_DATA_SCALE} -T p >> ${GEN_DATA_PATH}/logs/ssb.generate_data.${CHILD}.log 2>&1 &  
        mv ${GEN_DATA_PATH}/${CHILD}/date.tbl ${GEN_DATA_PATH}/${CHILD}/date.tbl.${CHILD}
        mv ${GEN_DATA_PATH}/${CHILD}/part.tbl ${GEN_DATA_PATH}/${CHILD}/part.tbl.${CHILD}
      fi
      if [ "$CHILD" -gt "1" ]; then
        touch ${GEN_DATA_PATH}/${CHILD}/date.tbl.${CHILD}
        touch ${GEN_DATA_PATH}/${CHILD}/part.tbl.${CHILD}
      fi
      CHILD=$((CHILD + 1))
    done
  else
