...
function check_variables() {
  ...
  echo "############################################################################"
  echo ""
  # shellcheck source=ssb_variables.sh
  source ./${VARS_FILE} 2> /dev/null
  if [ $? -ne 0 ]; then
    echo "./${VARS_FILE} does not exist. Please ensure that this file exists before running SSB. Exiting."
    exit 1
  fi
...
function source_bashrc() {
  if [ -f ${startup_file} ]; then
    ...
    echo "Error: ${startup_file} does not exist. Please ensure that this file is correctly set before running SSB. Exiting."
    exit 1
  fi
}
...
function print_log() {
  ...
  printf "%s|%s.%s|%s|%02d:%02d:%02d|%d|%d\n" ${id} ${schema_name} ${table_name} ${tuples} "$((S_DURATION / 3600 % 24))" "$((S_DURATION / 60 % 60))" "$((S_DURATION % 60))" "${T_START}" "${T_END}" >> ${SSB_DIR}/log/${logfile}
}
export -f print_log

function end_step() {
  local logfile=end_${1}.log
  touch ${SSB_DIR}/log/${logfile}
}
export -f end_step
...
function create_hosts_file() {
  ...
  if [ "$RUN_MODEL" == "local" ]; then
    ...
    psql ${PSQL_OPTIONS} -v ON_ERROR_STOP=1 -t -A -c "${SQL_QUERY}" -o ${SSB_DIR}/segment_hosts.txt
  fi
}
