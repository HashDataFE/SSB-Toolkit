...
LOG_FILE="${SSB_DIR}/log/rollout_load.log"
...
function get_file_count() {
  file_count=$(find ${SSB_DIR}/log -maxdepth 1 -name 'end_testing*' | grep -c . || true)
  echo "${file_count}"
}

rm -f ${SSB_DIR}/log/end_testing_*.log
rm -f ${SSB_DIR}/log/testing*.log
rm -f ${SSB_DIR}/log/rollout_testing_*.log
rm -f ${SSB_DIR}/log/*multi.explain_analyze.log
...
function generate_templates() {
  ...
  for i in $(seq 1 ${MULTI_USER_COUNT}); do
    ...
    rm -f "${sql_dir}"/*.sql
    echo "cp ${query_dir}/*.sql ${sql_dir}/"
    cp ${query_dir}/*.sql "${sql_dir}/"
  done
...
#  for i in $(find "${query_dir}" -maxdepth 1 -type f -name "*.sql" -printf "%f\n" | sort -n); do
...
#    echo "cp ${query_dir}/${i} ${sql_dir}/"
#    cp "${query_dir}/${i}" "${sql_dir}/"
#  done
}
...
for session_id in $(seq 1 ${MULTI_USER_COUNT}); do
  session_log="${SSB_DIR}/log/testing_session_${session_id}.log"
...
rm -f ${SSB_DIR}/log/end_testing_*.log # remove the counter log file if successful.
