...
function copy_tpc() {
#  cp ${PWD}/tools/qgen ../*_sql/
#  cp ${PWD}/tools/qgen ../*_multi_user/
  cp ${PWD}/tools/dbgen ../*_gen_data/
#  cp ${PWD}/tools/dists.dss ../*_sql/
#  cp ${PWD}/tools/dists.dss ../*_multi_user/
  cp ${PWD}/tools/dists.dss ../*_gen_data/

  #copy the compiled dsdgen program to the segment nodes when running in LOCAL mode
  if [ "${RUN_MODEL}" == "local" ]; then
    echo "copy ssb binaries to segment hosts"
    echo "RUN_MODEL is LOCAL, proceeding with copying binaries"
    for i in $(cat ${SSB_DIR}/segment_hosts.txt); do
      scp tools/dbgen tools/dists.dss ${i}: &
    done
    wait
  else
...
function copy_queries() {
  rm -rf ${SSB_DIR}/*_sql/query_templates
  rm -rf ${SSB_DIR}/*_multi_user/query_templates
  cp -R query_templates ${SSB_DIR}/*_sql/
  cp -R query_templates ${SSB_DIR}/*_multi_user/
}
